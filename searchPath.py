...
def UCS(width, height, start, target, config):
    ...
    while len(frontier)>0:
        min_frontierCost = min(frontierCost)
        index_of_min = frontierCost.index(min_frontierCost)
        node_cost = frontierCost.pop(index_of_min)
        node = frontier.pop(index_of_min)
        explored.append(node)
        if node[0]>0:
            # Go UP
            child = [node[0]-1,node[1]]
            ...
            if child not in frontier and child not in explored and config[child[0]][child[1]]!=-1:
                if child==target:
                    ...
                    break
                frontier.append(child)
                frontierCost.append(config[child[0]][child[1]] + node_cost)
                parent[childHash] = node
            elif child in frontier and config[child[0]][child[1]] + node_cost < frontierCost[frontier.index(child)]:
                child_index = frontier.index(child) 
                parent[childHash] = node
                frontierCost[child_index] = config[child[0]][child[1]] + node_cost
        if node[0]<height-1:
            # Go DOWN
            child = [node[0]+1,node[1]]
            childHash = float(str(child[0])+'.'+str(child[1]))
            ...
            # Replace the frontier with higher PATH-COST with the child!
            # But!!! I think it is useless in this problem because the cost is on the node!
            elif child in frontier and config[child[0]][child[1]] + node_cost < frontierCost[frontier.index(child)]:
                # find the index in frontier
                child_index = frontier.index(child) 
                # No change in the frontier, just change in the cost and its parent!
                parent[childHash] = node # Change the child's parent
                frontierCost[child_index] = config[child[0]][child[1]] + node_cost# Update the more optimal path cost of this child
